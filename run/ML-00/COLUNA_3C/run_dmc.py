    ...
    for one_versus_others in ['Hernia_vs_OT', 'Spondylolisthesis_vs_OT', 'Normal_vs_OT']:
        ...
        for realization in range(20):
            ...
            results['cf'].append((metric_results['ACCURACY'],
                                  metrics_calculator.confusion_matrix(list(y_test.reshape(y_test.shape[0])), y_out_dmc, labels=range(C)),
                                  classifier_dmc
                                  ))
        ...
        DataFrame(final_result).to_csv(get_project_root() + '/run/ML-00/COLUNA_3C/results/' + one_versus_others + '_result_dmc.csv')
