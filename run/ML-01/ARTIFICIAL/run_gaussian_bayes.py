...
if __name__ == '__main__':
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.savefig(get_project_root() + '/run/ML-01/ARTIFICIAL/results/' + 'dataset_artificial.png')
    plt.show()
    ...
    for realization in range(1):
        ...
        bayes_gaussian_clf.fit(train, features, 'y')
    ...
    for i in range(len(final_result['best_cf'])):
        ...
        path = get_project_root() + '/run/ML-01/ARTIFICIAL/results/'
    ...
    plot_dict = {
        'xx': xx,
        'yy': yy,
        'Z': array(bayes_gaussian_clf.predict(DataFrame(space, columns=features), features)),
        'classes': {}
    }
    ...
    coloring(plot_dict, ListedColormap(['#87CEFA', '#228B22', "#FF00FF"]), xlabel='x1', ylabel='x2',
             title='mapa de cores gaussian bayes', xlim=[-0.1, 1.1], ylim=[-0.1, 1.1],
             path=get_project_root() + '/run/ML-01/ARTIFICIAL/results/' + 'color_map_triangle_simple_net.jpg', save=True)
