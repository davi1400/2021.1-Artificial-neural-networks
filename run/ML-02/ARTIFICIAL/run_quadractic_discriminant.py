...
if __name__ == '__main__':
    ...
    for realization in range(20):
        ...
        quadratic_clf = quadraticDiscriminant(classes, features, 'y')
        quadratic_clf.fit(train)
        y_out = quadratic_clf.predict(test)
    ...
    plot_dict = {
        'xx': xx,
        'yy': yy,
        'Z': array(quadratic_clf.predict(DataFrame(space, columns=features))),
        'classes': {}
    }
    ...
    coloring(plot_dict, ListedColormap(['#87CEFA', '#228B22', "#FF00FF"]), xlabel='x1', ylabel='x2',
             ...
             path=get_project_root() + '/run/ML-02/ARTIFICIAL/results/' + 'color_map_triangle_QD.jpg', save=True)

    print(pd.DataFrame(final_result))
    pd.DataFrame(final_result).to_csv(get_project_root() + '/run/ML-02/ARTIFICIAL/results/' + 'result_QD.csv')
