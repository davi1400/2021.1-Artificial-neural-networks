...
if __name__ == '__main__':
    ...
    for realization in range(1):
        ...
        validation_alphas = linspace(0.015, 0.1, 20)
        sigmoid_net = hyperbolic_perceptron_network(epochs=1000, number_of_neurons=1, learning_rate=0.1, activation_function='sigmoid hiperbolic')
        sigmoid_net.fit(x_train, y_train, x_train_val, y_train_val, alphas=validation_alphas, validation=True)
        ...
        results['cf'].append((metric_results['ACCURACY'], metrics_calculator.confusion_matrix(y_test, y_out, labels=[-1, 1])))
    ...
    coloring(plot_dict, ListedColormap(['#FFAAAA', '#AAAAFF']), xlabel='x1', ylabel='x2',
             ...
             path=get_project_root() + '/run/TR-035/ARTIFICIAL/results/' + 'color_map_triangle_sigmoid_hyper_net.jpg', save=True)
    # print('dataset shape %s' % Counter(base[:, 2]))

    print(pd.DataFrame(final_result))
    # del final_result['best_cf']
    pd.DataFrame(final_result).to_csv(get_project_root() + '/run/TR-035/ARTIFICIAL/results/' + 'result_sigmoid_hyper_net.csv')
