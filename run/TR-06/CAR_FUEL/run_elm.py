...
if __name__ == '__main__':
    ...
    for realization in range(20):
        ...
        number_of_neurons = 2*np.arange(1, 50)
        simple_net = ExtremeLearningMachines(number_of_neurons=15, N_Classes=1, case='regression')
        simple_net.fit(x_train, y_train, x_train_val=x_train_val, y_train_val=y_train_val,
                       hidden=number_of_neurons, validation=False)
    ...
    pd.DataFrame(final_result).to_csv(
        get_project_root() + '/run/TR-06/CAR_FUEL/results/' + 'result_elm.csv')
