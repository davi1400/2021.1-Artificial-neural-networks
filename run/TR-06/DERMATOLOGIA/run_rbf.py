...
if __name__ == '__main__':
    ...
    for realization in range(20):
        ...
        validation_alphas = [0.15, 1.0, 2.0]
        hidden = [10, 15, 20]
        simple_net = RadialBasisFunction(number_of_neurons=15, N_Classes=6, alpha=1, case='classification')
    ...
    for i in range(len(final_result['best_cf'])):
        ...
        plt.title(
            'Matriz de connfusão dermatologia com alpha: ' + str(best_alpha) + ' e numero de neurônios: ' + str(best_number_centers))
    ...
    pd.DataFrame(final_result).to_csv(get_project_root() + '/run/TR-06/DERMATOLOGIA/results/' + 'result_rbf.csv')
